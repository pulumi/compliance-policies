...
describe("aws.lb.Listener.disallowUnencryptedTraffic", function() {
    const policy = policies.aws.lb.Listener.disallowUnencryptedTraffic;

    it("name", async function() {
        assertResourcePolicyName(policy, "aws-lb-listener-disallow-unencrypted-traffic");
    });
    ...
    it("#2", async function() {
        const args = getResourceValidationArgs();
        args.props.port = 80;
        await assertHasResourceViolation(policy, args, { message: "Load Balancers should now allow unencrypted (HTTP) traffic." });
    });
