...
export const disallowPreviewResource: ResourceValidationPolicy = policyManager.registerPolicy({
    resourceValidationPolicy: {
        ...
        validateResource: validateResourceOfType(ConfigurationService, (_, args, reportViolation) => {
            reportViolation(
                "Azure ConfigurationService shouldn't use an unstable API (appplatform.v20230101preview.ConfigurationService). A compatible replacement can be found at 'appplatform.ConfigurationService'."
            );
        }),
    },
    vendors: ["azure"],
