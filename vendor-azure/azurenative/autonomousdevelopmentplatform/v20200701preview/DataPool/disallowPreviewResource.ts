...
export const disallowPreviewResource: ResourceValidationPolicy = policyManager.registerPolicy({
    resourceValidationPolicy: {
        ...
        validateResource: validateResourceOfType(DataPool, (_, args, reportViolation) => {
            reportViolation(
                "Azure DataPool shouldn't use an unstable API (autonomousdevelopmentplatform.v20200701preview.DataPool). A compatible replacement can be found at 'autonomousdevelopmentplatform.DataPool'."
            );
        }),
    },
    vendors: ["azure"],
