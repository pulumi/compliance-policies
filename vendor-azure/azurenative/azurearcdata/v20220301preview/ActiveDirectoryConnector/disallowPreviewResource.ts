...
export const disallowPreviewResource: ResourceValidationPolicy = policyManager.registerPolicy({
    resourceValidationPolicy: {
        ...
        validateResource: validateResourceOfType(ActiveDirectoryConnector, (_, args, reportViolation) => {
            reportViolation(
                "Azure ActiveDirectoryConnector shouldn't use an unstable API (azurearcdata.v20220301preview.ActiveDirectoryConnector). A compatible replacement can be found at 'azurearcdata.ActiveDirectoryConnector'."
            );
        }),
    },
    vendors: ["azure"],
