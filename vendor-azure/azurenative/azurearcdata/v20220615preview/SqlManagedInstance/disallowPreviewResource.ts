...
export const disallowPreviewResource: ResourceValidationPolicy = policyManager.registerPolicy({
    resourceValidationPolicy: {
        ...
        validateResource: validateResourceOfType(SqlManagedInstance, (_, args, reportViolation) => {
            reportViolation(
                "Azure SqlManagedInstance shouldn't use an unstable API (azurearcdata.v20220615preview.SqlManagedInstance). A compatible replacement can be found at 'azurearcdata.SqlManagedInstance'."
            );
        }),
    },
    vendors: ["azure"],
