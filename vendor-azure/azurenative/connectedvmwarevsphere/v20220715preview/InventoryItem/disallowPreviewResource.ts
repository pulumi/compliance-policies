...
export const disallowPreviewResource: ResourceValidationPolicy = policyManager.registerPolicy({
    resourceValidationPolicy: {
        ...
        validateResource: validateResourceOfType(InventoryItem, (_, args, reportViolation) => {
            reportViolation(
                "Azure InventoryItem shouldn't use an unstable API (connectedvmwarevsphere.v20220715preview.InventoryItem). A compatible replacement can be found at 'connectedvmwarevsphere.InventoryItem'."
            );
        }),
    },
    vendors: ["azure"],
