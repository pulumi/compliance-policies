...
import { MachineExtension } from "@pulumi/azure-native/connectedvmwarevsphere/v20220715preview";
...
export const disallowPreviewResource: ResourceValidationPolicy = policyManager.registerPolicy({
    resourceValidationPolicy: {
        ...
        validateResource: validateResourceOfType(MachineExtension, (_, args, reportViolation) => {
            reportViolation(
                "Azure MachineExtension shouldn't use an unstable API (connectedvmwarevsphere.v20220715preview.MachineExtension). A compatible replacement can be found at 'connectedvmwarevsphere.MachineExtension'."
            );
        }),
    },
    vendors: ["azure"],
