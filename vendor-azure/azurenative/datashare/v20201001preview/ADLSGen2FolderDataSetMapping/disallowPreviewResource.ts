...
export const disallowPreviewResource: ResourceValidationPolicy = policyManager.registerPolicy({
    resourceValidationPolicy: {
        ...
        validateResource: validateResourceOfType(ADLSGen2FolderDataSetMapping, (_, args, reportViolation) => {
            reportViolation(
                "Azure ADLSGen2FolderDataSetMapping shouldn't use an unstable API (datashare.v20201001preview.ADLSGen2FolderDataSetMapping). A compatible replacement can be found at 'datashare.ADLSGen2FolderDataSetMapping'."
            );
        }),
    },
    vendors: ["azure"],
