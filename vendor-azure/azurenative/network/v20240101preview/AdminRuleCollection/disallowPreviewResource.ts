...
export const disallowPreviewResource: ResourceValidationPolicy = policyManager.registerPolicy({
    resourceValidationPolicy: {
        ...
        validateResource: validateResourceOfType(AdminRuleCollection, (_, args, reportViolation) => {
            reportViolation(
                "Azure AdminRuleCollection shouldn't use an unstable API (network.v20240101preview.AdminRuleCollection). A compatible replacement can be found at 'network.AdminRuleCollection'."
            );
        }),
    },
    vendors: ["azure"],
