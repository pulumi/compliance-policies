...
export const disallowAlphaResource: ResourceValidationPolicy = policyManager.registerPolicy({
    resourceValidationPolicy: {
        name: "googlenative-metastore-v1alpha-servicedatabaseiammember-disallow-alpha-resource",
        description: "Disallow the use of non-stable (Alpha) resouces (metastore.v1alpha.ServiceDatabaseIamMember).",
        enforcementLevel: "advisory",
        validateResource: validateResourceOfType(ServiceDatabaseIamMember, (_, args, reportViolation) => {
            reportViolation("Metastore ServiceDatabaseIamMember shouldn't use an unstable API (metastore.v1alpha.ServiceDatabaseIamMember).");
        }),
    },
    vendors: ["google"],
    services: ["metastore"],
