...
describe("gcp.compute.FirewallPolicyRule.disallowInboundHttpTraffic", function() {
    const policy = policies.gcp.compute.FirewallPolicyRule.disallowInboundHttpTraffic;

    it("name", async function() {
        assertResourcePolicyName(policy, "gcp-compute-firewallpolicyrule-disallow-inbound-http-traffic");
    });
    ...
    it("#2", async function() {
        const args = getResourceValidationArgs();
        args.props.match.layer4Configs[0].ports = ["80"];
        await assertHasResourceViolation(policy, args, { message: "Firewall Policy Rules should not allow ingress HTTP traffic." });
    });

    it("#3", async function() {
        const args = getResourceValidationArgs();
        args.props.match.layer4Configs[0].ports = undefined;
        await assertHasResourceViolation(policy, args, { message: "Firewall Policy Rules should not allow ingress HTTP traffic." });
    });

    it("#4", async function() {
        const args = getResourceValidationArgs();
        args.props.match.layer4Configs[0].ipProtocol = "udp";
        args.props.match.layer4Configs[0].ports = ["80"];
        await assertNoResourceViolations(policy, args);
    });
});
